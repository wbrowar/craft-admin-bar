...
| `returnUrl`     | *entry.url*   | Sets the URL that you are taken to after saving an entry. Defaults to the entry’s front-end URL (or the current request URL if the entry has none). Set this to `null` to redirect to the default `returnUrl` for the entry type. |
